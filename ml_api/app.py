...
topic_session = ort.InferenceSession(os.path.join(MODEL_DIR, 'hockey_nlp_model.onnx'))
...
def predict_topic():
    try:
        ...
        input_array = text_vector.toarray().astype(np.float32)

        outputs = topic_session.run(None, {'input': input_array})[0]
        exp_outputs = np.exp(outputs - np.max(outputs, axis=1, keepdims=True))
        probabilities = exp_outputs / exp_outputs.sum(axis=1, keepdims=True)
        predicted_class = np.argmax(probabilities, axis=1)

        pred_class = int(predicted_class[0])
        pred_probs = probabilities[0].tolist()
